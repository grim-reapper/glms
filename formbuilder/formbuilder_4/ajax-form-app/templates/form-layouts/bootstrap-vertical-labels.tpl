...
{if !$hide_form}
    ...
    {foreach from=$fields key=key item=value}
        ...
        {if $value.type neq 'hidden'}
            ...
            <div class="wrap spacer {if in_array($key, $same_row_fields)} small no_clear {else} to_clear {/if}">
                ...
                <label for="{$value.field_id}">{$value.text}</label>
            
                {* [START] INPUT FIELD *}
                {if $value.type eq 'input'}
                    <div><input {$value.attributes_html} type="text" name="{$value.name}" id="{$value.field_id}" value="{if $value.post_value}{$value.post_value}{else}{$value.default_value}{/if}" /></div>
                {/if}
                {* [END] INPUT FIELD *}
                ...
                {if $value.type eq 'textarea'}
                    <div><textarea {$value.attributes_html} name="{$value.name}" id="{$value.field_id}">{if $value.post_value}{$value.post_value}{else}{$value.default_value}{/if}</textarea></div>
                {/if}
                {* [END] TEXTAREA FIELD *}
    ...
    {if $c.captcha.enabled eq '1'}
        ...
        <div class="wrap spacer" id="{$form_id}_afb_main_sec_div">
            <label>{$c.security_code.text}</label>
            <div>
                <div id="{$form_id}_afb_input_box_div"><input size="{$c.security_code.size}" class="required text {$c.security_code.class}" type="text" id="{$form_id}_security_code" name="security_code" />&nbsp;&nbsp;</div> 
                <div id="{$form_id}_afb_captcha_div"><img width="{$c.captcha.width}" height="{$c.captcha.height}" border="0" id="{$form_id}_afb_captcha" class="afb_captcha_vertical" src="{$c.url.path_to_script}captcha.php?form_id={$form_id}&x={$x}" alt="" />&nbsp;<a id="{$form_id}_afb_captcha_refresh" href="#"><img id="afb_icon_refresh" border="0" alt="" width="16" height="16" src="{$c.url.path_to_script}images/icon-refresh.png" align="bottom" /></a></div>  
                <div class="clear"></div>
            </div>
        </div>
    ...
    <input id="{$form_id}_afb_submit_button" class="{$c.submit.class}" type="submit" name="submit" value="{$c.submit.button_text}" /><div id="{$form_id}_afb_ajax_loading">{$c.submit.submitting_text}</div><br />
