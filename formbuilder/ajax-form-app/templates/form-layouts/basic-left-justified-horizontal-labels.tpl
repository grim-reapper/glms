...
{if !$hide_form}
    ...
    {foreach from=$fields key=key item=value}
        ...
        {if $value.type neq 'hidden'}
            ...
            <div class="label_zone"><label>{$value.text}</label></div>
                ...
                {if $value.type eq 'textarea'}
                    <div class="field_zone"><textarea {$value.attributes_html} name="{$value.name}" id="{$value.field_id}">{if $value.post_value}{$value.post_value}{else}{$value.default_value}{/if}</textarea></div>
                {/if}
                {* [END] TEXTAREA FIELD *}
    ...
    <div class="afp_wrap">
        ...
        <label id="{$form_id}_scL">{$c.security_code.text}</label>
        </div>
