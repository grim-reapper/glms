<div class="table">
  <div class="head">
    <h5 class="iFrames"><?php echo "Fresh Dak List";?></h5>
          	 <?php
			 if($this->mdl_users->get_permission('dak_add')){
                $attributes = array('class' => 'basicBtn header_button','style' => ' margin-right: 290px;' );
                echo anchor('dak/add','Add New Dak',$attributes);
			 }
             ?>
  </div> 
...
<img src="<?php echo base_url(); ?>asset/images/plus.png" id="action_list_<?php echo $list->dak_pad_id ?>" onclick="list_action(<?php echo $list->dak_pad_id ?>);"   />
                       ...
                       if($this->mdl_users->get_permission('dak_print')){
						...
						if($this->mdl_users->get_permission('dak_delete')){
							echo ' <li>';
					    	echo anchor('dak/delete/'.$list->dak_pad_id ,'Delete');
							echo ' </li>';
						 }
						?>
                       </ul>
